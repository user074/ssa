...
def shaped_correctness_reward(answer: str, completion: str) -> tuple[float, float]:
    ...
    if len(tags["answer"]) > 0:
        is_correct = grade_answer(tags["answer"][-1], answer)
        if is_correct:
            reward = 80.0
            success = 1
    ...
    for b in range(batch_size):
        for g in range(grpo_size):
            text_completion = tokenizer.decode(
                completions[b, g].tolist(), skip_special_tokens=True
...
"""
Below are rewards grading copied from prm800k
Would be good to normalize the math due to latex formatting
#https://github.com/openai/prm800k/tree/main/prm800k/grading
"""
